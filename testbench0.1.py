...
# Apply more robust feature scaling
...
projected_dim = 128  # Further increased
nhead = 8
num_layers = 4  # Increased
output_dim = len(np.unique(y_train))
dropout = 0.3  # Slightly increased
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X_train)):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            accuracy = accuracy_score(y_val_fold, predicted)
...
transformer_encoder_output = model.transformer_encoder(model.input_projection(X_train_tensor) + model.positional_encoding).squeeze(1)
...
test_transformer_encoder_output = model.transformer_encoder(model.input_projection(X_test_tensor) + model.positional_encoding).squeeze(1)
...
transformer_proba = nn.functional.softmax(outputs, dim=1).numpy()
knn_proba = knn.predict_proba(test_transformer_encoder_output.detach().numpy())
ensemble_proba = (transformer_proba + knn_proba) / 2
ensemble_predicted = np.argmax(ensemble_proba, axis=1)
accuracy_ensemble = accuracy_score(y_test_tensor.numpy(), ensemble_predicted)
