...
def extract_features(model, X):
    with torch.no_grad():
        ...
        features = torch.cat([
            transformer_output[:, -1, :],  # Last token
            torch.mean(transformer_output, dim=1),  # Mean pooling
            torch.max(transformer_output, dim=1)[0]  # Max pooling
        ], dim=1)
...
# Ensemble prediction (Average of Transformer and best KNN)
transformer_proba = nn.functional.softmax(model(X_test_tensor), dim=1).cpu().numpy()
